...
import { ShipmentStatus, type ShipmentEvent } from '~/lib/client';
...
const statusColors = {
  ...
  processing: {
    ...
    outlineColor: 'outline-green-6 00',
  },
  in_transit: {
...
const statusIcons = {
  ...
  processing: <BicepsFlexed className="size-5 text-primary-foreground" />,
  in_transit: <Truck className="size-5 text-primary-foreground" />,
  out_for_delivery: (
    <SquareArrowOutUpRight className="size-5 text-primary-foreground" />
  ),
  ...
  returned: <Undo2 className="size-5 text-primary-foreground" />,
};
    ...
    <Card className="shadow-none">
      ...
      <CardContent className="grid gap-4">
        <div className="flex items-center space-x-4 rounded-xl p-[15px] bg-gray-100 relative">
          ...
          <div
            data-line
            className={`absolute left-[80px] top-[24px] bottom-0 w-0.5 ${statusColor.bgColor}`}
          />
          <div className="flex flex-col space-y-6 relative">
            <TimelineEvent
              ...
              outlineColor={statusColor.outlineColor}
            />
...
function TimelineEvent({
  ...
  return (
    <div className="flex items-center gap-x-[15px]">
      ...
      <div
        className={`w-[40px] h-[40px] ${bgColor} text-foreground flex items-center justify-center rounded-full ${hasOutline ? `outline-2 ${outlineColor} outline-offset-2` : ''}`}
      >
